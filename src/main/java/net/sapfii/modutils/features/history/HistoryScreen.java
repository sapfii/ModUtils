    ...
    public void addPunishment(PunishmentData punishment) {
        ...
        Text reasonText = Text.empty().append(Text.literal("◘ ").styled(style -> {
                ...
                .append(Text.literal("”").styled(style -> {
                    style = style.withColor(0xa7a8a7);
                    return style;}));
        List<OrderedText> wrappedReasonTexts = textRenderer.wrapLines(reasonText, 180);
        if (punishments.size() == 1) {
            historyScroll.addLine(Text.literal("                                                       ").withColor(0x555555).formatted(Formatting.STRIKETHROUGH).asOrderedText());
        }
        ...
        historyScroll.addLine(punishmentText.asOrderedText());
        if (punishment.reason.matches("No reason given\\.")) {
            ...
            for (OrderedText reasonLine : wrappedReasonTexts) {
